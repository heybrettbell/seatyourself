...
restaurant_list.each do |restaurant|
  restaurant = Restaurant.create!(name: restaurant[0], cuisine: restaurant[1], description: restaurant[2], capacity: restaurant[3], price_range[4], address: restaurant[5])
  puts "#{restaurant.name} created"
end

3.times do
  Reservation.create!(guests: 5, time: 6, date:, restaurant: Restaurant.first)
end

3.times do
  Reservation.create!(guests: 8, time: 6, date:, restaurant: Restaurant.last)
end
